...
ovl_plot_files <- file.path(plotdir, paste0(names(overlap_plots), "_expt_pred_overlaps.pdf"))
for (i in seq_along(overlap_plots)) {
  ggsave(overlap_plots[[i]], filename = ovl_plot_files[[i]], height = 3.5, width = 7, device = "pdf")
}
```
...
pred_plots <- lapply(cell_types, FUN = plotPredictorsVsExperiment, df = merged_quant,
                     pos_col = pos_col, pred_names_col = "pred_name_long", text_size = 13, ncol = 2)
...
es_scatters <- lapply(cell_types, FUN = predScatterPlots, df = merged_quant, y_col = "EffectSize",
                      ...
                      pred_names_col = "pred_name_long", ncol = 2)
...
merged_bins <- left_join(merged_quant, dist_bins, by = "name") %>% 
...
for (n in names(dist_prc_plots)) {
  ggsave(dist_prc_plots[[n]], filename = file.path(plotdir, paste0(n, "_distance_binned_prc.pdf")),
         height = plot_height, width = plot_width, device = "pdf")
}
```
