...
pred_config <- fread(here(snakemake@input$pred_config), colClasses = c("alpha" = "numeric"))
...
pred_config <- data.frame(
  pred_id = "baseline", pred_col = "distToTSS", boolean = FALSE, alpha = 1e4,
  aggregate_function = "mean", fill_value = Inf, inverse_predictor = TRUE,
  pred_name_long = "Distance to TSS") %>% 
  rbind(pred_config, .)
...
# add 'Regulated' column to merged data (only significant pairs that have negative effect size)
...
# get default alpha value for predictors that do not hava alpha specified (default = min alpha)
...
pred_plots <- lapply(cell_types, FUN = plotPredictorsVsExperiment, df = merged, pos_col = pos_col,
...
# make scatter plots
es_scatters <- lapply(cell_types, FUN = predScatterPlots, df = merged, y_col = "EffectSize",
...
merged_annot <- addPredictionClassLabels(merged, perf_summary = perf_summary, pos_col = pos_col)

# make one nice PR table
pr_df <- pr_dfs %>% 
  bind_rows(.id = "CellType") %>% 
  left_join(select(pred_config, pred_id, pred_col, pred_uid, pred_name_long), by = "pred_uid") %>% 
  select(pred_uid, pred_id, pred_col, CellType, pred_name_long,  alpha, precision, recall, F1)
