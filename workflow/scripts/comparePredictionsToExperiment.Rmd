...
# get minimum alpha value for predictors that do not have alpha specified (default = min alpha)
#pred_config <- getDefaultAlpha(pred_config, merged = merged)

# set colors for predictors in plots
...
perf_summary_print <- perf_summary %>% 
  ...
  select(cell_type, predictor = pred_name_long, AUPRC, max_F1, alpha_cutoff, sensitivity_at_cutoff,
         ...
         precision_at_min_sensitivity, alpha_at_min_sensitivity)

# save to text file
write_tsv(perf_summary_print,
...
perf_summary_print  %>% 
  ...
  formatStyle(c(2, 4, 7), `border-right` = "solid 1px")
...
merged_bins_filt <- filter(merged_bins, !`distToTSS (bins)` %in% invalid_bins)
...
dist_auprc <- applyCellTypes(merged_bins_filt, .fun = makePerfDistance, pred_config = pred_config,
                             min_sensitivity = snakemake@params$min_sensitivity, pos_col = pos_col,
                             bs_iter = 100)

# save distance prc plots to files
savePlotList(dist_auprc, basename = "distance_auprc.pdf", path = plotdir, height = 6, width = 8)
```

```{r, results='asis', fig.cap=cap, fig.height=6, fig.width=8}
# print plots for every cell type in tabs
...
dist_prc_plots <- applyCellTypes(merged_bins_filt, .fun = makePRCurveSubsets,
                                 ...
                                 pos_col = pos_col,
                                 min_sensitivity = snakemake@params$min_sensitivity,
                                 line_width = 1.2, point_size = 3.5, text_size = 13, nrow = nrow,
                                 colors = pred_colors)

# calculate plot dimensions based on number of features
plot_height <- nrow * 4
plot_width <- ceiling(n_bins / nrow) * 6
