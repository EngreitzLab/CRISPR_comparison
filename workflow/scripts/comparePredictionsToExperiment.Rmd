...
pred_config <- data.frame(
  pred_id = "baseline", pred_col = c("distToTSS", "closestTSS"), boolean = c(FALSE, TRUE),
  alpha = c(1e4, 1), aggregate_function = c("mean", "max"), fill_value = c(Inf, 0),
  inverse_predictor = c(TRUE, FALSE), pred_name_long = c("Distance to TSS", "Closest TSS")) %>% 
  rbind(pred_config, .)
