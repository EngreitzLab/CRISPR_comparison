...
# extract predicitons and experiment input files from config in snakemake object
...
```{r results='asis', fig.cap="Precision-recall curves", fig.height=4.5, fig.width=7.5, warning=FALSE}
...
```{r perfSummary, fig.cap = "Precision-recall summary"}
...
es_scatters <- lapply(cell_types, FUN = predScatterPlots, df = merged, y_col = "EffectSize",
                      point_size = 2, text_size = 13, pred_names_col = "pred_name_long")
...
```{r, results='asis', fig.cap="Predictors versus effect size", fig.height=plot_height, fig.width=plot_width}
# print plots for every cell type in tabs
for (i in cell_types){
  cat("##", i, '{.unlisted .unnumbered}', '\n', '<br>', '\n')
  plot(es_scatters[[i]])
